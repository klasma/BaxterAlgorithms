function [oList, oBlobSeq, oPHD] =...
    MigLogLikeList_PHD_ISBI_tracks(aBlobSeq, aImData, aMaxArcs, varargin)
% Computes migration scores for tracking using a GM-PHD filter.
%
% The function first runs a GM-PHD filter on the data, and then computes
% migration probabilities between pairs of Gaussian components in the PHDs
% of consecutive time points. For a pair of Gaussian components, the
% migration probability is conditioned on having a particle in the first of
% the two components. The first component is propagated forward in time and
% then the resulting PDF is computed at the mean of the second component.
% That value is then compared to the PDF of a distribution of other
% particles which is assumed to be uniform over the image. If the particles
% have a velocity associated with them, the velocity distribution of other
% targets will be an isotropic Gaussian. This method to compute migration
% scores is described in [1].
%
% Inputs:
% aBlobSeq - Cell array where cell t contains n array with all Blob
...
%
% See also:
% MigLogLikeList_generic, MigLogLikeList_PHD_ISBI_IMM, ComputeGMPHD, Track
...
for t = 1:length(aBlobSeq)-1
    ...
    for i = 1:n1
        ...
        % Exclude all migrations longer than 15 pixels to save computation.
        dist2 = sum((phd2.m(1:n_obs,:) - repmat(phd1.m(1:n_obs,i),1,phd2.J)).^2);
        ...
        for candIndex = 1:length(candidates)
            ...
            switch phd2.n
                ...
                case {4 6}  % constant velocity
                    % The velocity of other targets has the same distribution
                    % as appearing targets.
                    if phd2.n == 4
...
if aCreateOutputFiles
    % Save the migration probabilities to a mat-file.
