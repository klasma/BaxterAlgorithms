...
DataForExport = cell(length(wellsSR),length(timepointsSR)+1,length(ExportParamNames));

for i=1:length(ExportParamNames)
    DataForExport(:,1,i) = well_namesSR;
end

for j=1:size(wellsSR,2)% Number of wells in ND2 File 
    
        CurrSeries=wellsSR(j)-1; %The current well that we're looking at
        ...
        for i=1:length(timepointsSR) %For all of the time points in the series, should start at zero if T_Value has -1 built in, which it should
                    %Set up the particular timepoint image
            Timepoint = num2str(timepointsSR(i)-1,'%03.f'); %Creates a string so taht the BioFormats can read it
...
ExportFile=char('D:\Dropbox (VU Basic Sciences)\Duvall Confocal\Duvall Lab\Isa\2021-10-02-BigPRoteinScreen\2022-04-05-BF_Vasco\TestData.xlsx'); % blank spreadsheet for output data
if ~exist(exportdir,'file')
mkdir(exportdir);
end
%% Directory Code
