...
r.setId(ImgFile);
exportdir=char('D:\Dropbox (VU Basic Sciences)\Duvall Confocal\Duvall Lab\Brock Fletcher\2021-09-08\DoseResponse\mTmG\2022-04-26-Analysis');
...
MiPerPix=1.36; %The resolution of your microscopy, in Microns per pixel. A very important parameter. #PROJECT: I believe OME and BioFormats could pull this automagically        
...
%                 T_Value = 1
        ...
        %Therefore, would need to experiment with.
                       ...
                       %% Get Area and Intensity data for every combination of channels
                            [TidyFeat,TidyImg] = LabelAnalysis(LiveData,Img2,Well,Timepoint);
                                % #PROJECT: LabelAnalysis was written
                                % quickly and could likely be optimized to
                                % run faster. It's relatively slow.
                            Time{i+1}=[TidyFeat,TidyImg];
                            AllData2{i+1}={TidyFeat,TidyImg}; %Store Data in parfor-compatible way
                    end
                    AnaWell{j+1}=Time{i+1}
            AllData3(j+1,:)=AllData2; %Store Data in parfor-compatible way
            end
            ParWell{nn}=AnaWell{j+1}
            TidyFeat=TidyFeat;
...
[TPs] = CumCell(AllData4); %Custom Function to combine weird data format from parfor Loop
...
ExcelName=fullfile(RunDirectory,'patientdata.xlsx'); %Prepare excel file name
