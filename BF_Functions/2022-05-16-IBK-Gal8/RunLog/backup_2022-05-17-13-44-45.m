...
%                 T_Value = r2.getSizeT()-1; %Very important, the timepoints of the images. Returns the total number of timepoints, the -1 is important.
                ...
                SizeY=r2.getSizeY(); %Number of pixels in image in Y Dimension
                    ...
                    for i=0:T_Value %For all of the time points in the series, should start at zero if T_Value has -1 built in, which it should
                        Img2=zeros(SizeY,SizeX,numPlanes,'uint16','gpuArray');  %Make a blank shell for the images  
                                iplane=r2.getIndex(0,0,i);
                                for n=1:numPlanes             
                                        CurrImg= bfGetPlane(r2,iplane+n); %Creates a 3D matrix with every image color channel as a different layer. Matlab fast at matrixes
                                        ImMax=max(CurrImg,[],[1 2])
                                        Img2(:,:,n)=CurrImg.*(intmax(class(CurrImg))/ImMax);
                                end
                                Img2=Img2*(255)/(2^BitDepth);
                                Img2=uint8(Img2); %Converts Images to uint16 so they're always the same for downstream analysis.
