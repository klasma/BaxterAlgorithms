...
%Input Planes
    numPlanes=3; %Which image Planes to analyze ##Integrate with GUI 
     ImageAnalyses=    {
                        {{'Nuc'},{3},{0.8 100},{'Nuc_bw4_perim' [0.8500 0.3250 0.0980]}};
                        {{'Cyt'},{1},{0.8 100},{}};
                        {{'CytWS'},{},{},{}};
                        {{'Gal'},{1},{0.2},{}};
                        {{'Drug'},{2},{0.01},{}};
    ...
    for i=0:T_Value %For all of the time points in the series, should start at zero if T_Value has -1 built in, which it should
       ...
       Img=[];%Creates an empty array for the image ##Check and see if this is necessary or if there's a more efficient way of doing this.
            ...
            for k=1:length(ImageAnalyses)
                    ...
                    AnaImage=Img(:,:,ImageAnalyses{k,:}{2}{1});
                    ...
                    switch Analysis
                        ...
                        case 'Cyt'
                         [CytBright,CytArea,CytCytOverlay,cyt_bw4,CytPos,CytBrightEnough,CytMT1,CytOpen,cyt_eq,CytTopHat,cyt_bw4_perim] = Cytosol(cyt,CytMax,CytLow);   
                        case 'CytWS'
                    ...
                    if ~isempty(ImageAnalyses{k,:}{4})
                        RGBExportImage=imoverlay(RGBExportImage,str2func(ImageAnalyses{1,:}{4}{1}),ImageAnalyses{1,:}{4}{2});
                    end
         end
